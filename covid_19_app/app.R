...
cumulative[,1] <- as.Date(cumulative[,1], format = '%d/%m/%Y') 
cumulative_d[,1] <- as.Date(cumulative_d[,1], format = '%d/%m/%Y') 
...
server <- function(input, output) {
    ...
    output$hist <- renderPlotly({
        if(input$tab1cd == 'Cases'){
            p <- ggplot(cumulative, aes(Date, UK)) +
                geom_bar(stat = 'identity', color = 'black', fill = '#009E73', size = .25) +
                theme(axis.title = element_text(), axis.text.x = element_text(), axis.text.y = element_text())+
                scale_x_date(labels = date_format('%d-%m'))+
                ylab('Cases')
            ggplotly(p)
        } else{
            p <- ggplot(cumulative_d, aes(Date, Deaths)) +
                geom_bar(stat = 'identity', color = 'black', fill = '#009E73', size = .25) +
                theme(axis.title = element_text(), axis.text.x = element_text(), axis.text.y = element_text())+
                scale_x_date(labels = date_format('%d-%m'))+
                ylab('Deaths')
    ...
    output$uklog <- renderPlotly({
        if(input$tab1cd == 'Cases'){
            p <- ggplot(uk_2) + 
                geom_line(aes(x=Date, y=UK), color='#D55E00') +
                geom_point(aes(x=Date, y=UK), color='#D55E00') +
                ylab('Number of Cases')+
                theme(axis.title = element_text(), axis.text.x = element_text(), axis.text.y = element_text())+
                scale_x_date(labels = date_format('%d-%m'))+
